...
color.factor<-function(name.group,reps){
  ...
  if(length(name.group)==2){
    ...
    color.j <- I(colorRampPalette(brewer.pal(n=12, name = 'Set3'))(length(name.group)))
...
plot_PCA<-function(df,fname){
  ...
  df_out<-separate(df_out,col='rowname',into=c('cell','time','treat','rep'),sep = '_')
  #df_out$rowname2<-stringr::str_match(df_out$rowname, '(.*)_(.*)')[, -1]
  #df_out<-separate(df_out,col='rowname2',into=c('group','rep'))
  ...
  p<-ggplot(df_out, aes(x = PC1, y = PC2)) + 
    geom_point(aes(color = factor(time),shape=factor(treat)),size=4) + 
...
single.ANOVA <- function(df_sl,metadata,group,annot,df_ratio){
...
myVolcano<-function(data,p_threshold=0.05,fc_threshold=2,fname){
  ...
  up<-data%>%filter(pval<=p_threshold & (log2fc>log2(fc_threshold)))%>%
    filter_at(vars(17:18), any_vars(.>=2))%>%
    filter_at(vars(19:21), any_vars(.>=2))
  ...
  down<-data%>%filter(pval<=p_threshold & (log2fc<(-log2(fc_threshold))))%>%
    filter_at(vars(17:18), any_vars(.>=2))%>%
    filter_at(vars(19:21), any_vars(.>=2))
...
##----cluster plot or heatmap----------------
plot_heatmap <- function(sig,color){
  # make a heatmap of significant proteins
  # results-the result data frame
  
  temp<-data.matrix(sig)
  
  ## Row- and column-wise clustering 
  # cor.method = c("pearson", "kendall", "spearman")
  # dist.method ="euclidean", "maximum", "manhattan", "canberra", "binary" or "minkowski".
  #hclust.method="ward.D", "ward.D2", "single", "complete","average","median" 
  
  hr <- hclust(as.dist(1-cor(t(temp), method="pearson")), method="complete")
  hc <- hclust(as.dist(1-cor(temp, method="spearman")), method="complete")
  
  #hr <- hclust(as.dist(temp, method="euclidean"), method="complete")
  ...
  ## K mean cutting
  #kmcol <- pam(scale(temp),4)$clustering
  #RowSideColors=as.character(kmcol)
  
  tiff("cluster.Heatmap.tiff",width = 6, height = 5, units = 'in', res = 300, compression = 'lzw')
  heatmap.2(temp,scale = "row",col = bluered(100),trace = "none", density.info = "none",
            margins = c(8, 8),
            Rowv=as.dendrogram(hr), 
            Colv=as.dendrogram(hc),
            ColSideColors=as.character(myCol),RowSideColors=mycolhc)
  dev.off()
  cluster.No<-mycl
  cluster<-cbind(temp2,cluster.No)
  write_csv(cluster,"Clusters.sigprotein.csv")
